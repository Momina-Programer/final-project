...
const Order = ({order}) => {
    ...
    return (
     ...
     <p>Your Order has been palced successfully you will recieven an email confirmation shortly</p>
  
  <h3 className="text-lg font-semibold mb-2">Order Summary</h3>
  <p>Order Number: {order.orderNumber}</p>
  <div className="text-md font-semibold mb-2">
    <h2>Shopping</h2>
    <p>{order.shoppingInformation.address}</p>
    <p>{order.shoppingInformation.city}</p>
    <p>{order.shoppingInformation.zip}</p>
  </div>
  ...
  <div className="mt-4  flex justify-between">
    ...
    <span className="font-semibold">{order.totalPrice.toFiexd(2)}</span>
  </div>
  <div className="mt-6">
